...
def extract_approach_from_code(code: str, language: str) -> str:
    """
    Analyze code to detect the algorithmic approach
    This is a simple heuristic version - real implementation would use CodeBERT
    """
    code_lower = code.lower()
    
    # Simple pattern detection
    if "hashmap" in code_lower or "dict" in code_lower or "map<" in code_lower:
        return "Hash Table"
    elif "for" in code_lower and code.count("for") >= 2:
        return "Nested Loops (Brute Force)"
    elif "left" in code_lower and "right" in code_lower and "while" in code_lower:
        return "Two Pointers"
    elif "dp[" in code_lower or "memo" in code_lower:
        return "Dynamic Programming"
    elif "dfs" in code_lower or "recursion" in code_lower:
        return "Depth-First Search"
    else:
        return "Unknown Approach"


def estimate_complexity(code: str, language: str) -> Dict[str, str]:
    """
    Estimate time and space complexity
    Simple heuristic - real implementation would use static analysis
    """
    code_lower = code.lower()
    ...
    if "hashmap" in code_lower or "dict" in code_lower or "set" in code_lower:
        space = "O(n)"
    elif "[" in code and "new" in code_lower:
        space = "O(n)"
    else:
        space = "O(1)"
