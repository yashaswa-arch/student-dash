...
# Import analyzers (make ML optional)
analyzer = None
ml_available = False
try:
    # Heavy ML stack may not be installed on all machines
    ...
    # Fall back to lightweight LeetCode analyzer only
    print(f"⚠️ ML analyzers unavailable: {e}. Running in lightweight mode.")
...
# analyzer is initialized above if ML available
...
async def analyze_code(request: SimpleRequest):
    try:
        ...
        if not ml_available or analyzer is None:
            raise HTTPException(status_code=503, detail="ML analyzer not available on this server. Install optional ML dependencies or use /leetcode/analyze.")
...
async def mentor_analyze(request: MentorRequest):
    ...
    try:
        ...
        if not ml_available or analyzer is None:
            raise HTTPException(status_code=503, detail="Mentor analysis not available without ML dependencies.")
